...
async function displayCharacterSequence(figletFont, layout) {
  ...
  for (let frame = 0; frame <= totalFrames; frame++) {
    ...
    for (let charIndex = 0; charIndex < characterRanges.length; charIndex++) {
      ...
      if (charAlpha > 0) {
        // 現在アクティブな文字のインデックス
        const currentActiveChar = Math.floor(progress * characterRanges.length / 0.7);
        
        if (charIndex === currentActiveChar) {
          // 現在出現中の文字は最も明るく
          brightness = 0.3 + (charAlpha * 0.7);
        } else if (charIndex < currentActiveChar) {
          // 既に出現済みの文字は徐々に薄くなって最終的に消える
          const charsSinceAppeared = currentActiveChar - charIndex;
          const fadeAmount = charsSinceAppeared * 0.2; // より強いフェード
          
          if (fadeAmount >= 0.8) {
            // 完全に消す
            shouldRender = false;
          } else {
            brightness = Math.max(0.1, (0.3 + (charAlpha * 0.7)) - fadeAmount);
          }
        } else {
          // まだ出現していない文字
          brightness = 0.3 + (charAlpha * 0.7);
        ...
        // 最新の文字は特に明るく
        if (charIndex === currentActiveChar && charAlpha > 0.5) {
